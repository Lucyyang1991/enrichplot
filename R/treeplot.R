...
treeplot.compareClusterResult <-  function(x,
    ...
    ##############################################################
      ...
      if (!is.null(other_args$text.angle)) {
        text.angle = other_args$text.angle
      }
      else {
        text.angle = 60
      }
      if (!is.null(other_args$text.size)) {
        text.size = other_args$text.size
      }
      else {
        text.size = 3
      }
    ...
    if (!is.null(other_args$remove.category)) {
      y <- dplyr::filter(y, !ID %in% other_args$remove.category)
    }
    ...
    if (geneClusterPanel == "heatMap") {
        ...
        p <- p + ggnewscale::new_scale_fill() # +
            # coord_equal(xlim = xlim)
        p <- ggtree::gheatmap(p, ID_Cluster_mat, colnames_angle = text.angle,
                              font.size = text.size) +
    ...
    if (geneClusterPanel == "dotplot") {
        ...
        p <- p + ggnewscale::new_scale_colour() +
            ggtreeExtra::geom_fruit(data = dotdata, geom = geom_point,
                       ...
                       axis.params = list(axis = "x", text.size = text.size, line.alpha = 0,
                                          text.angle = text.angle, hjust = 1)) +
